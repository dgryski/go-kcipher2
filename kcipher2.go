...
func (k KeySizeError) Error() string { return "kcipher2: invalid key size " + strconv.Itoa(int(k)) }

type IVSizeError int

func (iv IVSizeError) Error() string { return "kcipher2: invalid iv size " + strconv.Itoa(int(iv)) }

type k2cipher struct {
	a              [5]uint32
	b              [11]uint32
	iv             [4]uint32
	ik             [12]uint32
...
// New returns a cipher.Stream implementing the KCipher-2 stream cipher.  The key and iv parameters must each be 16 bytes.
...
func (k *k2cipher) XORKeyStream(dst, src []byte) {
 ...
 */
func (k *k2cipher) stream() (uint32, uint32) {
